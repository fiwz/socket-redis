...
$('#btn-loginclient').click(function() {
    ...
    axios.post('http://localhost:4001/login-client', JSON.stringify(allData), {
        headers: {
...
function showLoginInfo() {
    fetch('http://localhost:4001/login-info',
    {
        method: "GET",
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include'
...
$(document).on('click', '#my-chats #pending ul li', function(e) {
    ...
    axios.get(`http://localhost:4001/chat-details/${chatId}`, {
        headers: {
            'Content-Type': 'application/json'
        },
        withCredentials: true,
