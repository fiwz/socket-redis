$(() => {
  ...
  $.get('/get_chatters', function (response) {
    ...
    $('.chat-info').text(
      'There are currently ' +
        response.data.numberOfChatters +
  ...
  $.get('/A/chats/pending', function (response) {
    ...
    for (let pd in response.data) {
      ...
      $('#my-chats #pending ul').append(
        `<li class="list-group-item" id="${response.data[pd]}" >${response.data[pd]}</li>`
      );
    }
  });
    ...
    $.get('/get_messages', function (response) {
      ...
      if (message_count > 0) {
        ...
        for (var x = 0; x < message_count; x++) {
          html +=
            "<div class='msg mb-2'><div class='user font-weight-bold'>" +
            response.data[x].sender +
            "</div><div class='txt'>" +
            response.data[x].message +
            '</div></div>';
        }
        $('#messages').html(html);
      }
